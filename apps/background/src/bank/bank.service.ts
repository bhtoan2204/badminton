      ...
      const results = await page
        .evaluate(() => {
          ...
          const interestRatesCounter = rowsQuay.map((row) => {
            ...
            const rateDetails = columns
              .slice(1)
              .map((td) => td.textContent.trim());
            if (rateDetails.length !== 10) {
              return null;
            }
            const rateObject = Object.fromEntries(
              rateDetails.map((rate, index) => [keys[index], rate]),
            );
            return {
        ...
        .then((results) => {
          return {
            interestRatesCounter: results.interestRatesCounter.filter(
              (result) => result !== null,
            ),
            interestRatesOnline: results.interestRatesOnline.filter(
              (result) => result !== null,
            ),
          };
        });
