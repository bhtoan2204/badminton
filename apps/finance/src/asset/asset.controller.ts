...
export class AssetController {
  ...
  async getAsset(@Payload() data: any, @Ctx() context: RmqContext) {
    ...
    return await this.assetService.getAsset(data.id_user, data.id_family, data.page, data.itemsPerPage);
  }
  ...
  async deleteAsset(@Payload() data: any, @Ctx() context: RmqContext) {
    this.rmqService.ack(context);
    return await this.assetService.deleteAsset(data.id_user, data.id_family, data.id_asset);
  }
}