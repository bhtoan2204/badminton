...
export class SubjectService {
  ...
  async createSubject(id_user: string, dto: CreateSubjectDto) {
    try {
      const response = this.educationClient
        .send('educationClient/createSubject', { id_user, dto })
        .pipe(timeout(15000));
  ...
  async getDetailSubject(
    ...
    try {
      const response = this.educationClient
        .send('educationClient/getDetailSubject', {
          id_user,
          id_family,
  ...
  async updateDetailSubject(id_user: string, dto: UpdateSubjectDto) {
    try {
      const response = this.educationClient
        .send('educationClient/updateDetailSubject', { id_user, dto })
        .pipe(timeout(15000));
      return await lastValueFrom(response);
    } catch (error) {
      if (error.name === 'TimeoutError') {
  ...
  async deleteSubject(
    ...
    try {
      const response = this.educationClient
        .send('educationClient/deleteSubject', {
          id_user,
          id_family,
          id_education_progress,
          id_subject,
  ...
  async addComponentScore(id_user: string, dto: AddComponentScoreDto) {
    try {
      const response = this.educationClient
        .send('educationClient/addComponentScore', { id_user, dto })
        .pipe(timeout(15000));
      return await lastValueFrom(response);
    } catch (error) {
      if (error.name === 'TimeoutError') {
  ...
  async insertComponentScore(id_user: string, dto: any) {
    try {
      const response = this.educationClient
        .send('educationClient/insertComponentScore', { id_user, dto })
        .pipe(timeout(15000));
      return await lastValueFrom(response);
    } catch (error) {
      if (error.name === 'TimeoutError') {
  ...
  async updateComponentScore(id_user: string, dto: UpdateComponentScoreDto) {
    try {
      const response = this.educationClient
        .send('educationClient/updateComponentScore', { id_user, dto })
        .pipe(timeout(15000));
      return await lastValueFrom(response);
    } catch (error) {
      if (error.name === 'TimeoutError') {
  ...
  async deleteComponentScore(id_user: string, dto: DeleteComponentScoreDto) {
    try {
      const response = this.educationClient
        .send('educationClient/deleteComponentScore', { id_user, dto })
        .pipe(timeout(15000));
      return await lastValueFrom(response);
    } catch (error) {
      if (error.name === 'TimeoutError') {
  ...
  async modifyScore(id_user: string, dto: ModifyScoreDto) {
    try {
      const response = this.educationClient
        .send('educationClient/modifyScore', { id_user, dto })
        .pipe(timeout(15000));
      return await lastValueFrom(response);
    } catch (error) {
      if (error.name === 'TimeoutError') {
  ...
  async removeScore(id_user: string, dto: any) {
    try {
      const response = this.educationClient
        .send('educationClient/removeScore', { id_user, dto })
        .pipe(timeout(15000));
      return await lastValueFrom(response);
    } catch (error) {
      if (error.name === 'TimeoutError') {
  ...
  async changeStatus(id_user: string, dto: any) {
    try {
      const response = this.educationClient
        .send('educationClient/changeStatus', { id_user, dto })
        .pipe(timeout(15000));
      return await lastValueFrom(response);
    } catch (error) {
      if (error.name === 'TimeoutError') {
